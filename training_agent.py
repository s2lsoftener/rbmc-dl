...
Description:    An agent modified to load a neural network and Stockfish engine from a passed in parameter,
                rather than loading a hardcoded ones.
                See the commented code around line 650 for enabling Stockfish's evaluation
                for expanding leaf nodes in MCTS.
...
def stockfish_pv(board: chess.Board, color: bool, engine: chess.engine.EngineProtocol):
    """
    Use stockfish to generate move probabilities and a value for the current state
    The output of the neural network is an 64x8x8 tensor.
    (The paper had 9 additional dimensions for underpromotions, but I'm just going to assume
    everything promotes to a queen.)
    The cells of each 8x8 plane denote where to pick up a piece,
    ...
    # value = np.clip(info['score'].relative.score() or 0, -1, 1)
    value = 0

    error = False

    # If there's an open king attack, we have to go for it. All other moves don't matter.
    moves = []
    scores = []
    enemy_king_square = board.king(not color)
    attackers = board.attackers(color, enemy_king_square) if enemy_king_square is not None else None
    if attackers:
        attacker_square = attackers.pop()
        moves = [chess.Move(attacker_square, enemy_king_square)]
        scores = [1]
        value = 1
    else:
        # Get an evaluation of each move
        # Run stockfish multipv to get an evaluation for each move
        try:
            # moves = list(board.pseudo_legal_moves) # stockfish hates analysing pseudo-legal moves
            moves = list(board.legal_moves)
            info = engine.analyse(board, limit, multipv=len(moves))
            moves = [i['pv'][0] for i in info]
            scores = np.array([i['score'].relative.score() or 0 for i in info])
            value = np.clip(scores, -1, 1).max() # Value is average of positive or negative advantage
            scores = softmax(scores)
        except:
            print('Stockfish ran into an error trying to evaluate the board.', color, board.fen())
            error = True
            moves = []
            scores = []
            value = 0

    # Get the piece corresponding to each move
    pieces = [board.piece_at(move.from_square) for move in moves]
    # Get the corresponding plane for each move and piece combination
    piece_planes = [map_move_plane(piece, move) for piece, move in zip(pieces, moves)]

    probs = torch.zeros((64, 8, 8)) # 64x8x8
    for move, plane, score in zip(moves, piece_planes, scores):
        x0, y0 = chess.square_file(move.from_square), chess.square_rank(move.from_square)
        probs[plane, y0, x0] = score
    return probs, value, error
# %% MCTS
...
class TrainingAgent(Player):    
    def __init__(self, nnet: AlphaZeroChess, engine: chess.engine.EngineProtocol) -> None:
        self.nnet = nnet
        self.engine = engine
        return
    ...
    def handle_game_start(self, color, board):
        ...
        self.turn_time_left = self.turn_time_max # I only want to use 30 seconds per turn.
