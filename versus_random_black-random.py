...
def episode(nnet: AlphaZeroChess, engine: chess.engine.EngineProtocol):
    ...
    player1 = TrainingAgent(nnet, engine) # white
    player2 = Random() # black
...
if __name__ == '__main__':
    ...
    # Load neural net
    checkpoint = torch.load('k-random.pt', map_location=lambda device, loc: device)
    nnet = AlphaZeroChess()
    nnet.to(device)
    ...
    for i in range(int(args.start), int(args.end)):
        ...
        win_percent = (np.array(winners) == True).sum() / len(winners)
        print("!!!!AlphaZero has won percentage of games:", str(win_percent))
        joblib.dump(data, './output/k-12-1-random-wVSb-random-' + str(i) + '.joblib')
        # outputxt = open("12-1-20_az-random-0_vs_randblack.txt", "a")
        outputxt = open(args.target, "a")
